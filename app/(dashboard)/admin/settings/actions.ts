...
export async function getPublicAppSettings(): Promise<{ contactEmail: string; emailNotificationsEnabled: boolean } | null> {
...
export async function updateAppSettings(formData: FormData): Promise<{ success: boolean; error?: string }> {
    const session = await auth();
    if (!session?.user || session.user.role !== 'admin') {
...
// Usuń wiadomość
export async function deleteContactMessage(messageId: string): Promise<{ success: boolean }> {
    const session = await auth();
    if (!session?.user || session.user.role !== 'admin') {
        return { success: false };
    }
