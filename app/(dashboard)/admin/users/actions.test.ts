 ...
 *
 * Testuje:
 * - Autoryzację dla operacji admina (checkAdmin)
...
describe("Admin Users Actions", () => {
  ...
  describe("toggleBlockUser", () => {
    it("powinien rzucić Unauthorized dla niezalogowanego użytkownika", async () => {
      await expect(toggleBlockUser(validUserId, true)).rejects.toThrow(
        "Unauthorized",
      );
    });

    it("powinien rzucić błąd dla nieprawidłowego statusu blokady", async () => {
      await expect(toggleBlockUser(validUserId, "not-boolean")).rejects.toThrow(
        "Nieprawidłowy status blokady",
      );
    });
    ...
    it("powinien zablokować użytkownika gdy admin", async () => {
      ...
      await expect(toggleBlockUser(validUserId, false)).resolves.not.toThrow();
    ...
    it("powinien odblokować użytkownika gdy admin", async () => {
      ...
      await expect(toggleBlockUser(validUserId, true)).resolves.not.toThrow();
  ...
  describe("toggleUserRole", () => {
    it("powinien rzucić Unauthorized dla niezalogowanego użytkownika", async () => {
      await expect(toggleUserRole(validUserId, "user")).rejects.toThrow(
        "Unauthorized",
      );
    });

    it("powinien rzucić błąd dla nieprawidłowej roli", async () => {
      await expect(toggleUserRole(validUserId, "superadmin")).rejects.toThrow(
    ...
    it("powinien zmienić rolę gdy admin", async () => {
      mockAdmin();
  ...
  describe("deleteUser", () => {
    it("powinien rzucić Unauthorized dla niezalogowanego użytkownika", async () => {
      await expect(deleteUser(validUserId)).rejects.toThrow("Unauthorized");
    });

    it("powinien rzucić Forbidden dla nie-admina", async () => {
      mockNonAdmin();

      await expect(deleteUser(validUserId)).rejects.toThrow("Forbidden");
    });

    it("powinien usunąć użytkownika gdy admin", async () => {
      mockAdmin();
  ...
  describe("createUser", () => {
    ...
    it("powinien rzucić Unauthorized dla niezalogowanego", async () => {
      await expect(createUser(validUserData)).rejects.toThrow("Unauthorized");
    });

    it("powinien rzucić Forbidden dla nie-admina", async () => {
      mockNonAdmin();

      await expect(createUser(validUserData)).rejects.toThrow("Forbidden");
    });
    ...
    it("powinien utworzyć użytkownika gdy dane poprawne i admin", async () => {
      ...
      await expect(createUser(validUserData)).resolves.not.toThrow();
  ...
  describe("deleteUserData", () => {
    it("powinien rzucić Unauthorized dla niezalogowanego", async () => {
      await expect(deleteUserData(validUserId)).rejects.toThrow("Unauthorized");
    });
  ...
  describe("deleteUserHistory", () => {
    it("powinien rzucić Unauthorized dla niezalogowanego", async () => {
      await expect(deleteUserHistory(validUserId)).rejects.toThrow(
        "Unauthorized",
      );
    });
  ...
  describe("resetUserProgress", () => {
    it("powinien rzucić Unauthorized dla niezalogowanego", async () => {
      await expect(resetUserProgress(validUserId)).rejects.toThrow(
        "Unauthorized",
      );
    });
