 ...
 * Wszystkie powiązane dane są usuwane automatycznie dzięki CASCADE w schemacie bazy.
 * Po usunięciu czyści sesję SSO.
 *
...
export async function deleteMyAccount() {
  ...
  try {
    ...
    // Dzięki ON DELETE CASCADE wszystkie powiązane dane zostaną automatycznie usunięte:
    // - accounts, sessions, learningSessions, wordProgress, achievements, userStats, customWords
    await db.delete(users).where(eq(users.id, currentUserId));

    // Wyczyść sesję SSO (wylogowanie)
