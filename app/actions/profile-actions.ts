...
export async function updateUserName(rawName: unknown): Promise<ActionResult> {
  ...
  if (!validationResult.success) {
    ...
    return {
      success: false,
      error: firstError?.message || "Nieprawidłowa nazwa",
    };
  }
...
export async function updateUserEmail(
  ...
  return {
    success: false,
    error:
      "Zmiana adresu email jest możliwa tylko w centrum logowania. Po zmianie w centrum, email zostanie automatycznie zaktualizowany przy następnym logowaniu.",
  };
}
