 ...
 */
export async function checkActionsRateLimit(identifier: string): Promise<RateLimitResult> {
    if (hasUpstash) {
    ...
    return inMemoryRateLimit(`actions:${identifier}`, requests, windowToMs(window));
}
