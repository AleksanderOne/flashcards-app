...
describe('SSO Client', () => {
    ...
    describe('clearSSOSession', () => {
        it('powinien usunąć ciasteczko sesji', async () => {
            const mockCookieStore = {
                delete: vi.fn(),
            };
            (cookies as any).mockResolvedValue(mockCookieStore);
    ...
    describe('verifySessionWithCenter', () => {
        ...
        it('powinien zwrócić false przy błędzie HTTP', async () => {
            ...
            const result = await verifySessionWithCenter(testUserId, testTokenVersion);
    ...
    describe('getCallbackUrl', () => {
        it('powinien poprawnie generować URL callbacku', () => {
            ...
            expect(result).toBe('https://flashcards.example.com/api/auth/sso-callback');
        });
