...
vi.mock("./sso-client", () => ({
  ...
  getSSOConfig: vi.fn().mockResolvedValue({
    centerUrl: "https://sso.example.com",
    apiKey: "test-api-key",
...
describe("Auth Admin Strict (Fail-Closed)", () => {
  ...
  describe("authAdminStrict", () => {
    ...
    it("powinien zwrócić null gdy brak konfiguracji SSO", async () => {
      // Mockujemy getSSOConfig żeby zwracał puste wartości
      const { getSSOConfig } = await import("./sso-client");
      (getSSOConfig as any).mockResolvedValueOnce({
        centerUrl: "",
        apiKey: "",
        projectSlug: "",
      });
