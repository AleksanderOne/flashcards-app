...
import { getSSOSession, getSSOConfig } from "./sso-client";
...
async function verifySessionWithCenterStrict(
  ...
  // Używamy getSSOConfig() dla spójności z resztą kodu SSO
  // (pobiera z bazy danych z fallbackiem na process.env)
  const config = await getSSOConfig();
  const centerUrl = config.centerUrl;
