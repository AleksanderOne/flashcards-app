...
export function FloatingNav({ children, className }: { children: React.ReactNode; className?: string }) {
...
const faqItems = [
    {
        ...
        answer: 'Tak! Flashcards jest całkowicie darmową platformą. Nie ma żadnych ukrytych opłat, subskrypcji ani reklam. Wszystkie funkcje są dostępne dla każdego użytkownika bezpłatnie.',
    },
    {
        question: 'Jak działa algorytm Spaced Repetition?',
        answer: 'Algorytm SM-2 automatycznie planuje powtórki słówek w optymalnych odstępach czasowych. Po pierwszym spotkaniu ze słówkiem, system zaplanuje powtórkę na następny dzień. Każda poprawna odpowiedź wydłuża przerwę (1 dzień → 3 dni → tydzień → miesiąc itd.), dzięki czemu słówka przechodzą do pamięci długotrwałej.',
    },
    {
        question: 'Ile czasu dziennie powinienem poświęcić na naukę?',
        answer: 'Zalecamy 15-30 minut dziennie. Regularność jest ważniejsza niż długość sesji. Krótkie, codzienne sesje są bardziej efektywne niż rzadkie, wielogodzinne maratony nauki.',
    },
    {
        question: 'Jakie poziomy trudności są dostępne?',
        answer: 'Oferujemy słówka na wszystkich poziomach CEFR: od A1 (Beginner) do C1 (Advanced). Możesz wybrać poziom odpowiadający Twojemu aktualnemu zaawansowaniu i stopniowo przechodzić na wyższe.',
    },
    {
        question: 'Czy mogę dodawać własne słówka?',
        answer: 'Tak! Możesz tworzyć i zarządzać własnymi zestawami słówek. Dodawaj słowa, które chcesz się nauczyć, a system Spaced Repetition będzie je automatycznie planować do powtórek.',
    },
    {
        question: 'Na jakich urządzeniach działa aplikacja?',
        answer: 'Flashcards jest aplikacją webową, która działa na wszystkich urządzeniach z dostępem do internetu - komputerach, tabletach i smartfonach. Nie musisz instalować żadnej aplikacji.',
    },
];
...
export function ContactForm() {
    ...
    return (
        ...
        >
            <form action={formAction} className="p-8 rounded-3xl bg-card/80 backdrop-blur-sm border-2 border-violet-200 dark:border-violet-800 shadow-xl">
                {state.success ? (
                    <motion.div
                        initial={{ scale: 0.8, opacity: 0 }}
